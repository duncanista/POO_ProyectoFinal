...
Class Venta{
  ...
  function insert($fecha, $total, $pago, $cambio, $idUsuario){
    $sql = "INSERT INTO Venta(fecha, total, pago, cambio, idUsuario) VALUES('$fecha', '$total', '$pago', '$cambio', idUsuario)";
    if(simpleQuery($sql)){
      $response["status"] = 201;
  ...
  function update($id, $fecha, $total, $pago, $cambio, $idUsuario){
    $sql = "UPDATE Venta SET fecha='$fecha', total='$total', pago='$pago', cambio='$cambio', idUsuario=$idUsuario WHERE idVenta=$id";
    if(simpleQuery($sql)){
      $response["status"] = 201;
